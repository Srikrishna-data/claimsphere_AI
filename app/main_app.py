...
main_app.py
-------------
This is the entire orchestration "graph" for ClaimSphere AI.

Notice there is no LangGraph, no StateGraph, no special workflow library.
The flow is just:

    result_1 = run_agent_1(...)
    result_2 = run_agent_2(...)
    if result_2["fraud_score"] is low:
        result_3 = run_agent_3(...)
    pdf_path = run_agent_4(result_1, result_2, result_3, final_status)

That sequence of function calls IS the workflow. Each function takes the
previous function's output dict as input. This is what people mean by
"agents passing state to each other" - it's just passing a dictionary
as an argument.
...
# ---------------------------------------------------------------------------
# App setup - runs once when the app starts
# ---------------------------------------------------------------------------
init_db()
seed_sample_data()
...
# ---------------------------------------------------------------------------
# Very simple login - good enough for a demo, NOT secure for production.
# For production you'd use a real auth provider (Auth0, Supabase Auth, etc).
# ---------------------------------------------------------------------------
def login_screen():
    st.title("ClaimSphere AI - Login")
